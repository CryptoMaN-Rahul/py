...
class RegExpToENFAConverter:
    ...
    def print_adj_list():
        ...
        for i in range(len(RegExpToENFAConverter.enfa)):
            edge_list = RegExpToENFAConverter.enfa[i]
            print(i)
            for edge in edge_list:
                print(edge.transition, "->", edge.state)

    @staticmethod
    def create_adj_matrix():
        RegExpToENFAConverter.matrix_dimension = len(RegExpToENFAConverter.enfa)
        RegExpToENFAConverter.enfa_matrix = [[None] * RegExpToENFAConverter.matrix_dimension for _ in range(RegExpToENFAConverter.matrix_dimension)]
        for i in range(len(RegExpToENFAConverter.enfa)):
            edge_list = RegExpToENFAConverter.enfa[i]
            for edge in edge_list:
                RegExpToENFAConverter.enfa_matrix[i][edge.state] = edge.transition

    @staticmethod
    def print_adj_matrix():
        print("Adjacency Matrix")
        print(" ", end="\t")
        for i in range(RegExpToENFAConverter.matrix_dimension):
            print(i, end="\t")
        print()
        for i in range(RegExpToENFAConverter.matrix_dimension):
            print(i, end="\t")
            for j in range(RegExpToENFAConverter.matrix_dimension):
                print(RegExpToENFAConverter.enfa_matrix[i][j], end="\t") 
            print()
    ...
    def regex_to_enfa(postfix_regex):
        ...
        print("Initial State:", RegExpToENFAConverter.initial_state)
        print("Final State:", RegExpToENFAConverter.final_state)
        RegExpToENFAConverter.create_adj_matrix()
        RegExpToENFAConverter.print_adj_list()
        print()
        RegExpToENFAConverter.print_adj_matrix()
